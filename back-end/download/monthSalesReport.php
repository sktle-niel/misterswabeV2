...
header('Content-Disposition: attachment; filename="month_sales_report_' . date('Y-m') . '.pdf"');
...
$pdf->Cell(0, 10, 'Monthly Sales Report - ' . date('F Y'), 0, 1, 'C');
$pdf->Ln(5);
...
// Get PDF content
$pdfContent = $pdf->Output('month_sales_report_' . date('Y-m') . '.pdf', 'S');

// Save to report history
$reportName = date('F Y') . ' Sales Report.pdf';
$filePath = 'month_sales_report_' . date('Y-m') . '.pdf';
$fileSize = getReportFileSize($pdfContent);
$period = date('F Y');

saveReportHistory($conn, 'monthly_sales', $reportName, $filePath, $fileSize, $period, $pdfContent);
// Output the PDF
echo $pdfContent;
?>