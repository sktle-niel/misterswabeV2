<?php
// Default values for the delete modal (only if not already set)
if (!isset($modalId)) $modalId = 'deleteModal';
if (!isset($title)) $title = 'Delete Product';
if (!isset($message)) $message = 'Are you sure you want to delete this product? This action cannot be undone.';
if (!isset($cancelText)) $cancelText = 'Cancel';
if (!isset($confirmText)) $confirmText = 'Delete';
if (!isset($confirmFunction)) $confirmFunction = 'confirmDelete';
if (!isset($closeFunction)) $closeFunction = 'closeDeleteModal';
?>

<!-- Delete Confirmation Modal -->
    <div class="modal-overlay" id="<?php echo $modalId; ?>Overlay" onclick="<?php echo $closeFunction; ?>OnOverlay(event)" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 10000;">
